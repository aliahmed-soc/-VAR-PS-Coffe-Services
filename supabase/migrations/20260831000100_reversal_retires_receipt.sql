...
-- Repair orders the old function already left inconsistent: reversed, back in
-- checkout_pending, but still carrying the retired receipt. Their repayment
-- event can never be accepted until the receipt is cleared. Narrow and
-- idempotent: a checkout_pending order can only hold a receipt via that bug.
UPDATE orders o
SET receipt_number = NULL,
    receipt_snapshot = NULL
WHERE o.status = 'checkout_pending'
  AND o.receipt_snapshot IS NOT NULL
  AND EXISTS (
    SELECT 1 FROM payments p
    WHERE p.order_id = o.id AND p.payment_type = 'reversal'
  );
