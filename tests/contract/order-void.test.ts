...
describe("order.voided cloud contract", () => {
  ...
  it("writes one sale_void movement per line", () => {
    ...
    expect(voided).toMatch(/oi\.status = 'active'/);
  });
  ...
  it("clears the product value the void throws away", () => {
    ...
    expect(voided).toMatch(/subtotal_minor = gaming_subtotal_minor/);
  });
  ...
  it("leaves the per-line void path alone", () => {
    ...
    expect(item).toMatch(/quantity_on_hand = quantity_on_hand \+ \(p_payload->>'quantity'\)::integer/);
  });
});
