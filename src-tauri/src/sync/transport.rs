...
pub async fn apply_domain_event(
    ...
    let url = format!("{}/rest/v1/rpc/apply_domain_event", cfg.url.trim_end_matches('/'));
    let res = client
        .post(url)
    ...
    let body = res.text().await.map_err(|e| AppError::Sync(e.to_string()))?;
...
pub async fn pull_branch_since(
    ...
    let client = reqwest::Client::new();
    let url = format!("{}/rest/v1/rpc/pull_branch_since", cfg.url.trim_end_matches('/'));
    let res = client
        .post(url)
        .header(AUTHORIZATION, format!("Bearer {access_token}"))
        .header("apikey", &cfg.anon_key)
...
pub fn env_config() -> Option<SupabaseConfig> {
    ...
    if url.contains("supabase.co") && cfg!(debug_assertions) && std::env::var("PSC_ALLOW_PROD").ok().as_deref() != Some("1")
    {
